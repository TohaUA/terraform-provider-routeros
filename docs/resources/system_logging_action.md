...
- `managed` (Boolean) Read-only status flag reported by RouterOS 7.24 showing whether the logging action is managed by another subsystem. The property is not described in the MikroTik documentation and is exposed here only so that it stops being dropped during the schema conversion.
